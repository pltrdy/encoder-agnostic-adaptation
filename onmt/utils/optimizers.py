...
def build_torch_optimizer(model, opt):
    ...
    generator_params= [n for n, pp in model.generator.named_parameters()]
    ggp = [n for n, p in model.generator.named_parameters() if p.requires_grad]
    print("Generator params: ", ", ".join(generator_params))
    print("Generator params [req grad]: ", ", ".join(ggp))
    assert ggp == generator_params
    named_params = ["%s %s" % (str(n), str(p.requires_grad)) for n, p in model.named_parameters()]
   
    model_named_params = [n for n, p in model.named_parameters()]
    for gnp in generator_params:
        _gnp = "generator.%s" % gnp
        print(gnp, "/", _gnp)
        if _gnp not in model_named_params:
            print("EXCLUDED: ", _gnp)

    print("generator: ", model.generator.linear)
    print("generator: ", model.generator.linear.weight)
    print("#optim params: %d/%d" % (len(params), len(named_params)))
    path = 'optim_params.%d' % (len(params))
    with open(path, "w") as f:
        print("Writing optim params to '%s'" % path)
        print("\n".join(named_params), file=f)
